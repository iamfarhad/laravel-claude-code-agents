...
- `task_open`: orchestrator only. Fields task_id, workflow, prd_path for code-changing flows; optional mr_url + reviewed_head_sha (full immutable hash), risk_gates (reviewer names), and optional base_sha. One task/session; scope cannot silently change.
  base_sha sets the task's diff scope and must be a full hash of a commit present in this checkout. It defaults to the local HEAD, which is correct for a working-tree flow but WRONG for an MR review: there the checkout is already at the reviewed head, so the default makes `context kind=diff` empty and leaves `derivedRiskGates` with nothing to classify. For an MR task pass the merge-base with the target branch, which `fetch_mr` returns as `metadata.base_sha`. Because `task_status` is available to every role, this is also how a specialist that is not granted `fetch_mr` obtains a usable diff scope.
